...
There are broadly speaking, two types of chromatography:
- **Preparative chromatography** targets isolation and purification of "large" quantities of a substance.
- **Analytical chromatography** serves to identify or quantify analytes in a mixture.

Mostly, chromatographic models are used for optimization of preparative processes.

+++ {"slideshow": {"slide_type": "fragment"}}

For modeling these processes, we have to combine all of the techniques we learned in the previous lessons:
...
Only the bed porosity and axial dispersion affect the Dextran pulse.
...
- Flow rate: $10^{-6}/60~\text{m}^3~\text{s}^{-1}$

+++ {"slideshow": {"slide_type": "fragment"}}

Moreover, since Dextran does not penetrate pores, the film diffusion coefficient can be set to $0~\text{m} \text{s}^{-1}$.
...
- In the first section, which lasts $50~\text{s}$, the concentration of Dextran at the inlet is $1.0~\text{mM}$,
...
This time, use two `Inlet UnitOperations`, a feed `Inlet` with a concentration of $10~\text{mM}$ for both components and an eluent `Inlet` with a concentration of $0~\text{mM}$.
...
Often, multiple `Events` happen simultaneously.
...
You can see all the dependent events in the `dependent_events` attribute:

```{code-cell} ipython3
---
editable: true
---
print(process.dependent_events)
```
...
### A note on reference concentrations

```{note}
From a practical perspective, modern resins have a very high capacity and large proteins can have a very high characteristic charge.
...
we obtain a (partial) non-dimensionalization of the adsorption and desorption rates:
...
The protein is loaded for $7500~\text{s}$, then there is a wash step, which takes $2000~\text{s}$, and the gradient takes another $5500~\text{s}$.
...
_ = simulation_results.solution.column.outlet.plot(secondary_axes=sec)
```
