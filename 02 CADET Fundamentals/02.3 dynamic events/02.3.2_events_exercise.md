...
binding_model.adsorption_rate = [0, 1e-3, 1, 1e-2]
...
process.add_event("load_start", "flow_sheet.inlet.c", [[50], [1], [1], [1]], time=0)
process.add_event("wash_start", "flow_sheet.inlet.c", [[100], [0], [0], [0]], time=10)
process.add_event("elute_start", "flow_sheet.inlet.c", [[100, 900/gradient_length, 0, 0], [0], [0], [0]], time=column.volume / flow_rate * 3 + 10)
process.add_event("elute_end", "flow_sheet.inlet.c", [[1000, 0, 0, 0], [0], [0], [0]], time=column.volume / flow_rate * 3 + 10 + gradient_length)

process.cycle_time = column.volume / flow_rate * 3 + 10 + gradient_length + 100
```

Try plotting the events of the process with `process.plot_events()`

```{code-cell} ipython3
#process.plot_events()
```

If we plot the simulation results sim_results.solution.outlet.outlet.plot() is will not be very informative, as it plots all components on the same y-axis, even though they are of vastly different magnitudes. Use this code to generate a secondary y-axis to plot the salt concentration on:
