...
Let us start working towards the task given by our colleagues. Try to set up your column dimensions to match the data from the lab book (found in Workshop/LabBook/LabBook.pdf).
...
```
is_kinetic = False
adsorption_rate = [0, 1e-3, 1e0, 1e-2]
...
Use a feed concentration of $c_{feed} = [50, 10, 10, 10]~mol \cdot m^{-3} $.
...
binding_model.is_kinetic = False
binding_model.adsorption_rate = [0, 1e-3, 1e0, 1e-2]
