...
It tells us how long a small particle is likely to to spend inside a system.
...
- Set up our first unit operation models manually.
...
The results of inlet and oulet can be plotted as per usual, but in the oulet we spot, that something is off:
...
```{warning}
Because of numerical dispersion, solvers like CADET are not suited to simulate stiff systems like the one presented, were we set the axial dispersion to zero.
...
```{note}
In this workshop, we use CADET with a finite volume scheme for the spatial discretization.
However, by now most unit operation methods also support *Discontinuous Galerkin* which can increase speed substantially. Further information on DG can be found [here](https://cadet-process.readthedocs.io/en/v0.11.1/user_guide/process_model/unit_operation.html#discretization).
```
