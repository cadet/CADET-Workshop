...
For this exercise, we will expand our previous fit to include the measurements of a conductivity sensor. We will also include the replicate that our colleagues in the lab made to control for system variance. You can find all required meta-data in the labbook and the measurement data in `./experimental_data`.
...
Inspect the conductivity measurements and see what kind of pre-processing might make them usefull and which unit operation you can compare them to.
...
`flow_sheet.column.binding_model.characteristic_charge` from `1` to `30`
