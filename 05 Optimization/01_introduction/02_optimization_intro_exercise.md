...
optimization_problem.add_variable('x', lb=2)
...
def constrained_cubic(x):
    ...
    -------
    float, bool
        The value of the objective function at x and a boolean indicating if the constraint is met.
    """
    # Objective function (cubic)
    obj_val = (x - 2)**3 - 3 * (x - 2) + 1

    return obj_val, constraint_met
...
optimizer.optimize(optimization_problem, x0=3)
```

```{code-cell} ipython3
def rastrigin(x):
    """
    Compute the 1D Rastrigin function.

    Parameters
    ----------
    x : float
        The variable for the Rastrigin function.
