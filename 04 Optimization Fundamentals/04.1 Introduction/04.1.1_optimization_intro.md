...
For demonstration purposes, consider a quadratic function to be minimized.
...
The `OptimizationProblem` class is used to specify optimization variables, objectives, and constraints.
...
As an example, let us consider the constrained minimization of the Rosenbrock function (adapted from [SciPy](https://docs.scipy.org/doc/scipy/tutorial/optimize.html#id34)):
...
To set up this problem, first a new `OptimizationProblem` is created and the variables are added, including bounds.
It is important to note that `x0` cannot be used as a variable name since it is reserved for the initial values.
...
The `add_linear_constraint` function takes the variables subject to the constraint as the first argument.
The left-hand side $a$ and the bound $b_a$ are passed as the second and third arguments.
...
In **CADET-Process**, each row $a_{eq}$ of the constraint matrix needs to be added individually.
The `add_linear_equality_constraint` function takes the variables subject to the constraint as the first argument.
The left-hand side $a_{eq}$ and the bound $b_{eq, a}$ are passed as the second and third arguments.
It is important to note that the column order in $a$ is inferred from the order in which the optimization variables are passed.
...
In contrast to linear constraints, and analogous to objective functions, nonlinear constraints need to be added as callable functions.
...
print(rosenbrock_optimization_results.x)
