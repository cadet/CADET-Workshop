...
c_salt_gradient1_end = 200
c_salt_gradient2_end = 500
duration_gradient1 = 3127.53243
t_gradient1_start = 90
t_gradient2_start = 40*60
t_start_wash = 10
gradient_1_slope = (c_salt_gradient1_end - c_salt_gradient1_start)/(t_gradient2_start - t_gradient1_start)
gradient_2_slope = (c_salt_gradient2_end - c_salt_gradient1_end)/(process.cycle_time - t_gradient2_start)
...
process.add_event("grad2_start", "flow_sheet.inlet.c", [[c_salt_gradient1_end, gradient_2_slope, 0, 0], [0], [0], [0], ], time=t_gradient2_start)
```
...
Here, component $B$ seems to have sufficient purity between 38 minutes and 45 minutes and component $C$ between 50 and 70 minutes.
...
tags: [solution]
---
fractionator.add_fractionation_event('start_B', 1, 38*60, 'outlet')
fractionator.add_fractionation_event('end_B', -1, 45*60)
fractionator.add_fractionation_event('start_C', 2, 50*60)
fractionator.add_fractionation_event('end_C', -1, 70*60)
```

+++ {"editable": true, "slideshow": {"slide_type": "slide"}}

The `performance` property of the `Fractionator` contains the performance information:
...
For comparison, this is the results if all three components are relevant:
...
for key in fractionator.performance_keys:
    print(key, ":", getattr(fractionator, key))
...
tags: [solution]
---
# Note: currently, with the linear equality constraint, hopsy can struggle to find initial values to sample the polytope from. Therefore, don't execute this line.

# optimization_problem.add_linear_equality_constraint(['grad1_start_time', 'grad1_duration.time', 'grad2_start.time'], lhs=[1, 1, -1], beq=0, eps=1e-8)
```
...
tags: [solution]
---
print(optimization_problem.check_linear_equality_constraints([10, 50, 0.1, 100, 110, 100, 0.1]))
...
We could also use this to specify the `grad2_start.time` instead of using a linear equality constraint:
